...
interface Wave { y: number; amplitude: number; frequency: number; speed: number; offset: number; color: string; opacity: number }
const waves = ref<Wave[]>([])

let frameCount = 0

function createWaves(w: number, h: number): Wave[] {
  return Array.from({ length: 5 }, (_, i) => ({
    y: h * 0.15 + i * h * 0.12, amplitude: 20 + Math.random() * 40,
    frequency: 0.003 + Math.random() * 0.004, speed: 0.005 + Math.random() * 0.01,
    offset: Math.random() * Math.PI * 2, color: AURORA_COLORS[i % AURORA_COLORS.length],
...
function animate(c: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
  ...
  for (const w of waves.value) {
    ...
    const grad = ctx.createLinearGradient(0, w.y - w.amplitude * 2, 0, w.y + c.height * 0.3)
    grad.addColorStop(0, w.color + w.opacity + ')'); grad.addColorStop(0.5, w.color + w.opacity * 0.5 + ')'); grad.addColorStop(1, w.color + '0)')
...
function resize() {
  ...
  waves.value = createWaves(canvasRef.value.width, canvasRef.value.height)
}
