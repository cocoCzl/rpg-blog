<template>
  <div class="space-y-2">
    ...
    <div v-for="eff in effects" :key="eff.key" class="flex items-center gap-2 py-1.5 px-2 rounded-lg border text-xs transition-all" :style="{ background: getBg(eff.effectType), borderColor: getBorder(eff.effectType) }">
      ...
      <span v-if="eff.isActive" class="ml-auto w-1.5 h-1.5 rounded-full animate-pulse flex-shrink-0" :style="{ background: eff.color || getTextColor(eff.effectType) }" :aria-label="eff.isActive ? eff.name + ' is active' : undefined" />
    </div>
  </div>
</template>
